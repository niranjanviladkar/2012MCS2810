...
svm_predictions_file=`echo $projectRoot/libsvm-3.17/matlab/Hollywood2.train.predictions`
...
decv_file=`echo $projectRoot/libsvm-3.17/matlab/Hollywood2.train.decv`

# This file contains true labels for testing clips.
# This file comes with the dataset.
true_train_labels=`echo $projectRoot/dataset/Hollywood/ClipSets/all_labels_train.txt`

# This is the directory which contains output of object detector 
obj_train_dir=`echo $projectRoot/detectedOBJ/Hollywood2/train`
...
do
	train_file=`tail -n+$i $training_names_file | head -n1`
	decv_line=`tail -n+$i $decv_file | head -n1`
	num_cols=`echo $decv_line | wc -w`
#	output_file=`echo train_db/$train_file.db`

	true_lbl_line=`cat $true_train_labels | grep $train_file`
	line1=`echo $true_lbl_line | cut -f2- -d ' '`

#	rm -f $output_file 2>/dev/null
	echo Processing $train_file.avi... \( $i out of $total \)
	...
	if [ $debug -eq 0 ]; then
		...
		while [ ! -z "$true_lbl_line" ]; do
			...
			if [ "$this_lbl" = "1" ]; then
				t=$(int_to_str $j)
				echo "HasActivity(\"$train_file\",$t)" >> $output_file
			fi
	...
	while [ $j -le $num_cols ]; do
		...
		echo | awk -v n1=$decv_val -v n2=$t -v file=$train_file '{ 
		if( n1 < -2 )
...
#		for j in "${objModels[@]}"
#		do
#			grep -m 1 $j $obj_train_dir/$train_file.avi >/dev/null 2>&1
#			temp=$?
#			if [ $temp -eq 0 ]; then
#				echo "ObjPresent(\"$train_file\",\"$j\")" >> $output_file
#			fi
#		done	
#	fi

	if [ $processObj -eq 1 ]; then
		cat $obj_train_dir/$train_file.avi | while read line
	do
		case "$line" in
			*FRAME*)
				;;
			*)
				...
				echo | awk -v file=$train_file -v o=$obj -v s=$score -v t=$threshold '{
					if( s >= t )
