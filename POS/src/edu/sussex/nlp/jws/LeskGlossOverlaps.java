...
public class LeskGlossOverlaps
{
	...
	private boolean					usestoplist		= true;																																																																																																																																																																																																																																																																																																																															// default:
																																																																																																																																																																																																																																																																																																																																														// 'on'
	private boolean					uselemmatiser	= true;																																																																																																																																																																																																																																																																																																																															// default:
																																																																																																																																																																																																																																																																																																																																														// 'on'
	...
	private boolean containsContentWords( List < String > overlap )
	{
		...
		if ( usestoplist && uselemmatiser ) // both
		{
			for ( String w : overlap )
			{
				...
				if ( !lemma.isEmpty() && !stoplist.contains( w ) ) // is *not*
																	// on stop
																	// list and
																	// *is
																	// WordNet*
	...
	private ArrayList < List < String >> builder( ArrayList < String > words )
	{
		...
		for ( int i = 0; i < words.size(); i++ )
		{
			for ( int j = 0; j < words.size(); j++ )
			{
				if ( j + k <= words.size() )
				{
					build.add( 0, words.subList( j, ( j + k ) ) ); // put the
																	// longest
																	// length
																	// candidates
																	// at the
	...
	public double overlap( String x, String y )
	{
		...
		if ( x.equals( y ) )
		{
			if ( containsContentWords( xlist ) ) // check that the x gloss does
													// have at least 1 content
													// word
			{
		...
		for ( List < String > o : store )
		{
			...
			{
				totalscore += ( Math.pow( (double) o.size(), 2.0 ) ); // squre
																		// the
																		// length
																		// of
																		// the
