...
public class Lin
{
	...
	// 'lin' Computes the relatedness of two word senses using an information
	// content scheme.
	// The relatedness is equal to twice the information content of the LCS
	// divided by the sum of the information content of each input synset.
	// Parameters: 2 word senses in "word#pos#sense" format
	/*
	 * Semantic Relatedness_Lin = 2 * IC(lcs) / (IC(synset1) + IC(synset2)).
	 * Where IC(*) is the information content of *. The retuned value will be in
	 * the range 0 ... 1
	 * 
	 * If IC(synset1) or IC(synset2) is zero, then zero is returned as the
	 * relatedness score, due to lack of data. Ted's observation: Ideally, the
	 * information content of a synset would be zero only if that synset were
	 * the root node, but when the frequency of a synset is zero, we use the
	 * value of zero as the information content because of a lack of better
	 * alternatives.
	 */
	public double lin( String w1, int s1, String w2, int s2, String pos )
	{
		...
		// ...........................................................................................................................................
		// get the {synsets}
		IWordID word1ID = word1.getWordIDs().get( s1 - 1 ); // get the right
															// sense of word 1
		ISynset synset1 = dict.getWord( word1ID ).getSynset();
		// System.out.println(synset1);

		IWordID word2ID = word2.getWordIDs().get( s2 - 1 ); // get the right
															// sense of word 2
		...
		// [error check] If IC(synset1) or IC(synset2) is zero, then zero is
		...
		{
			ic3 = icfinder.getIC( null, pos ); // not strictly necessary, but is
	...
	public TreeMap < String, Double > lin( String w1, String w2, String pos )
	{
		...
		if ( word1 != null && word2 != null )
		{
			...
			List < IWordID > word1IDs = word1.getWordIDs(); // all senses of
															// word 1
			List < IWordID > word2IDs = word2.getWordIDs(); // all senses of
															// word 2
			int sx = 1;
	...
	// getLCS - get the LCS (least common subsumer) <hypernym> or, indeed,
	// {synset} itself!
	// the <hypernym> | {synset} with the highest Information Content (if there
	// is more than one
	// with the highest IC, we just get any of the 'tied' synsets as we are onlu
	// using the value
	public ISynset getLCS( ISynset synset1, ISynset synset2, String pos )
	{
		...
		getHypernyms( s1, h1 );
		// !!! important !!! we must add the original {synset} back in, as the 2
		// {synsets}(senses) we are comparing may be equivalent i.e. bthe same
		// {synset}!
		...
		// get the candidate <lcs>s i.e. the intersection of all <hypernyms> |
		// {synsets} which subsume the 2 {synsets}
		h1.retainAll( h2 );
		if ( h1.isEmpty() )
		{
		...
		for ( ISynsetID h : h1 )
		{
			double ic = icfinder.getIC( "" + h.getOffset(), pos ); // use
																	// ICfinder
																	// to get
																	// the
																	// Information
		...
		return ( dict.getSynset( maxlcs ) ); // return the <synset} with *a*
												// highest IC value
	}
	...
	public static void main( String[] args ) throws IOException
	{
		...
		double linscore = lin.lin( "apple", 1, "pear", 1, "n" ); // "word1",
																	// sense#,
																	// "word2",
																	// sense#,
																	// "POS"
		System.out.println( "specific senses" );
		// if(linscore != 0) // 0 is an error code i.e it means that something
		// isn't right e.g. words are not in WordNet, wrong POS etc
		// {
		System.out.println( "lin:\t" + formatter.format( linscore ) );
		// }
		// System.out.println();
		/*
		 * // lin(2) all senses: a value (score) of 0 is an error code for a
		 * pair TreeMap<String,Double> map = lin.lin("live", "banana", "n"); //
		 * "word1", "word2", "POS" System.out.println("all senses"); for(String
		 * pair : map.keySet()) { EasyIn.pause(pair + "\t" +
		 * formatter.format(map.get(pair))); } System.out.println();
		 * 
		 * // max value (i.e. highest score!) : get the highest score for 2
		 * words double maxvalue = lin.max("live", "banana", "n"); // "word1",
		 * "word2", "POS" System.out.println("max value");
		 * System.out.println(formatter.format(maxvalue)); System.out.println();
		 * 
		 * // lin(3) all senses of word 1 vs. a specific sense of word 2: a
		 * value (score) of 0 is an error code for a pair TreeMap<String,Double>
		 * map3 = lin.lin("live", "banana", 2, "n"); // "word1", "word2",
		 * sense#, "POS"
		 * System.out.println("all senses of word 1 vs. fixed sense of word 2");
		 * for(String pair : map3.keySet()) { EasyIn.pause(pair + "\t" +
		 * formatter.format(map3.get(pair))); } System.out.println();
		 * 
		 * // lin(4) a specific sense of word 1 vs. all senses of word 2: a
		 * value (score) of 0 is an error code for a pair TreeMap<String,Double>
		 * map4 = lin.lin("live", 1, "banana", "n"); // "word1", sense#,
		 * "word2", "POS"
		 * System.out.println("fixed sense of word 1 vs. all senses of word 2");
		 * for(String pair : map4.keySet()) { EasyIn.pause(pair + "\t" +
